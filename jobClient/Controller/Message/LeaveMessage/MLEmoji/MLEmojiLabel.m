...
@property (nonatomic, strong) NSArray *customEmojiArray;
...
+ (NSArray *)emojiArray {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        NSString *emojiFilePath = [[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:@"emoticons.plist"];
...
//        NSDictionary *emojiDict = self.customEmojiRegularExpression?self.customEmojiDictionary:[MLEmojiLabel emojiDictionary];
        NSArray *emojiArray = self.customEmojiArray;
        //如果当前获得key后面有多余的，这个需要记录下
        NSMutableAttributedString *otherAppendStr = nil;
        NSString *imageName;
        for (NSDictionary *dict in emojiArray) {
            if ([dict[@"chs"] isEqualToString:emojiKey]) {
                imageName = dict[@"png"];
                break;
            }
        }
...
//接受NSMutableAttributedString
- (void)setEmojiAttributedText:(NSMutableAttributedString*)emojiTextAttributedString emojiText:(NSString *)emojiText
{
    ...
    [self setTextWithColor:mutableAttributedString name:emojiTextAttributedString afterInheritingLabelAttributesAndConfiguringWithBlock:nil];
    NSRange stringRange = NSMakeRange(0, emojiTextAttributedString.length);
    ...
    for (NSUInteger i=0; i<maxIndex; i++) {
        ...
        [regexps[i] enumerateMatchesInString:[emojiTextAttributedString string] options:0 range:stringRange usingBlock:^(NSTextCheckingResult *result, __unused NSMatchingFlags flags, __unused BOOL *stop) {
...
//	    self.customEmojiDictionary = [[NSDictionary alloc] initWithContentsOfFile:emojiFilePath];
        self.customEmojiArray = [NSArray arrayWithContentsOfFile:emojiFilePath];
    }else{
//        self.customEmojiDictionary = nil;
        self.customEmojiArray = nil;
    }
