...
//                UIImage *img = [UIImage imageWithCGImage:hiresCGImage scale:1.0 orientation:UIImageOrientationUp]
...
- (CGSize)cropSize
{
    if(_cropSize.width == 0 || _cropSize.height == 0) {
        _cropSize = CGSizeMake(ScreenWidth,ScreenWidth);
    }
    return _cropSize;
}
...
- (void)setSourceImage:(UIImage *)sourceImage
{
    if(sourceImage != _sourceImage)
    {
        ...
        self.cropSize = [self creatHeightWidthSize:_sourceImage.size andSize:CGSizeMake(ScreenWidth,ScreenHeight-100)];
    }
}
...
- (void)updateCropRect
{
    ...
    frame.size = size;
    frame.origin.x = (self.frameView.bounds.size.width-frame.size.width)/2;
    frame.origin.y = (self.frameView.bounds.size.height-frame.size.height)/2;
...
- (IBAction)doneAction:(id)sender
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            self.view.userInteractionEnabled = YES;
            [self endTransformHook:transform];
        });
    });
        ...
        case UIGestureRecognizerStateEnded: {
            ...
            if(self.gestureCount == 0) {
                ...
                if(scale != self.scale) {
                    ...
                    self.view.userInteractionEnabled = NO;
                    [UIView animateWithDuration:kAnimationIntervalTransform delay:0 options:UIViewAnimationOptionLayoutSubviews animations:^{
...
- (IBAction)rotateImge:(id)sender
{
    ...
    self.imageView.transform = transform;
    //self.cropSize = [self creatHeightWidthSize:self.imageView.frame.size andSize:CGSizeMake(320,380)];
}
