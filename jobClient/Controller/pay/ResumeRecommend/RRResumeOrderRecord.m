...
@implementation RRResumeOrderRecord
...
{
    [con getOrderRecord:[Manager getUserInfo].userId_ pageSize:15 serviceCode:@"P_ZHITUI" gwcFor:@"20" pageIndex:requestCon_.pageInfo_.currentPage_];
}
...
{
    static NSString *CellIdentifier = @"RRResumeOrderCell";
    
    RRResumeOrderCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
    if (cell == nil) {
        cell = [[[NSBundle mainBundle] loadNibNamed:@"RRResumeOrderCell" owner:self options:nil] lastObject];
...
-(void)backBarBtnResponse:(id)sender
{
    ...
    for ( UIViewController *ctl in self.navigationController.viewControllers ) {
        if( [ctl isKindOfClass:[RRServiceInfo class]] ){
            tmpCtl = ctl;
            break;
        }
    }
