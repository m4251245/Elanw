...
- (NSMutableAttributedString*)attributedTextForTextStorageFromLabelProperties
{
    ...
    for (MLLink *link in self.links) {
        ...
        if ([link isEqual:self.activeLink]) {
            ...
            if (!attributes) {
//                attributes = @{NSForegroundColorAttributeName : UIColorFromRGB(0x456487),NSBackgroundColorAttributeName:kDefaultActiveLinkBackgroundColorForMLLinkLabel};
                attributes = @{NSBackgroundColorAttributeName: UIColorFromRGB(0xbdbdbd)};
...
//                attributes = @{NSForegroundColorAttributeName:self.linkColorForMLLinkLabel};
//            }
        }
//        [attributedString addAttributes:attributes range:link.linkRange];
    }
