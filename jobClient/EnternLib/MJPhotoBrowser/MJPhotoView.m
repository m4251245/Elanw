...
//        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(delegateResumePhoto) name:@"DELEGATEIMAGE" object:nil];
    }
    return self;
}
            ...
            if (_photo.url == nil) {
                ...
                [_imageView sd_setImageWithURL:_photo.url placeholderImage:_photo.placeholder options:SDWebImageRetryFailed|SDWebImageLowPriority progress:^(NSInteger receivedSize, NSInteger expectedSize) {
                    if (receivedSize > kMinProgress) {
                        if (expectedSize != 0) {
                            loading.progress = (receivedSize/(expectedSize*0.1));
                        }
                    }
...
- (UIView *)viewForZoomingInScrollView:(UIScrollView *)scrollView {
    scrollView.contentSize = CGSizeMake(CGRectGetMaxX(_imageView.frame),CGRectGetMaxY(_imageView.frame));
	return _imageView;
}
...
- (void)handleDoubleTap:(UITapGestureRecognizer *)tap {
    ...
    self.contentSize = CGSizeMake(CGRectGetMaxX(_imageView.frame),CGRectGetMaxY(_imageView.frame));
}
