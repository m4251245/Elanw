...
#pragma clang diagnostic pop
    ...
    if (*xIndex == 5*self.scrollView.frame.size.width) {
        self.skipButton = [[UIButton alloc] initWithFrame:CGRectMake(pageView.frame.size.width/2-42, [UIScreen mainScreen].bounds.size.height - 60, 93 , 25)];
        ...
        [self.skipButton setBackgroundImage:[UIImage imageNamed:@"640-96020"] forState:UIControlStateNormal];
        [pageView addSubview:self.skipButton];
    }
    else
    {
        CGRect btnFrame = CGRectMake(self.scrollView.frame.size.width - 50, self.scrollView.frame.size.height - 75, 40, 25) ;
...
//    [self addSubview:self.pageControl];
    ...
    //NSLog(@"%f  %f",self.scrollView.contentSize.width-self.scrollView.frame.size.width/2,self.pageControl.frame.origin.y-10);
